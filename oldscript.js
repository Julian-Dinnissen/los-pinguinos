async function fetchData() {
  ...
  document.addEventListener("DOMContentLoaded", async function () {
    ...
    const ctx9 = document.getElementById("bekering").getContext("2d");
    ...
    try {
      ...
      const berkering = new Chart(ctx9, {
        type: "line",
        data: {
          labels: years,
          datasets: [
            {
              label: "Bekeerde pinguïns",
              data: Array(years.length).fill(0), // Initialize data with zeros
              backgroundColor: "rgba(255, 99, 132, 0.2)",
              borderColor: "rgba(255, 99, 132, 1)",
              borderWidth: 1,
      ...
      function animateGraph(graph) {
        graph.data.datasets[0].data[yearIndex] = populationData[yearIndex];
        graph.update();
      }
